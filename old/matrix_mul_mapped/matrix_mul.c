...
void mm_alloc( struct matrices *mm )
{
	...
	CU_SAFE_CALL( cuMemHostAlloc( (void**)(&mm->h_C), nsize*sizeof(int),
		flags ) );
	CU_SAFE_CALL( cuMemHostGetDevicePointer( (CUdeviceptr*)(&mm->d_A),
		mm->h_A, 0) );
	CU_SAFE_CALL( cuMemHostGetDevicePointer( (CUdeviceptr*)(&mm->d_B),
		mm->h_B, 0) );
	CU_SAFE_CALL( cuMemHostGetDevicePointer( (CUdeviceptr*)(&mm->d_C),
		mm->h_C, 0) );
}
