...
main(int argc, char** argv)
{
	...
	float time1_0, time2_0, time3_0, time4_0, k_time;
	...
	CUDA_SAFE_CALL( cudaMalloc( (void**)&d, mem_size ) );
	CUDA_SAFE_CALL( cudaMemcpy( d, h, mem_size, cudaMemcpyHostToDevice ));
	...
	for( i= 0; i < max_iter; i++ ){
		...
		time4_0= (t4.tv_sec-t0.tv_sec)*1e6+(t4.tv_usec-t0.tv_usec);
		...
		fprintf( stdout, "%10u %10u %10.2f %10.2f %10.2f %10.2f %10.2f\n",
			mem_size, N, time1_0, time2_0, time3_0, time4_0, k_time );
		fflush(stdout);
	}
	}
