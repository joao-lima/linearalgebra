...
////////////////////////////////////////////////////////////////////////////////
#ifndef SMEM_ATOMICS
    typedef volatile uint histogram_t;
    inline __device__ void addByte(histogram_t *s_WarpHist, uint data, uint threadTag){
        uint count;
        do{
    ...
    typedef uint histogram_t;
    inline __device__ void addByte(histogram_t *s_WarpHist, uint data, uint threadTag){
        atomicAdd(s_WarpHist + data, 1);
    }
#endif

inline __device__ void addWord(histogram_t *s_WarpHist, uint data, uint tag){
...
__global__ void histogram256Kernel(uint *d_PartialHistograms, uint *d_Data, uint dataCount){
    ...
    __shared__ histogram_t s_Hist[HISTOGRAM256_THREADBLOCK_MEMORY];
