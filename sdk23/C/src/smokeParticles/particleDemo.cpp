...
float particleLifetime = numParticles / emitterRate;
...
void initParticles(int numParticles, bool bUseVBO)
{
    psystem = new ParticleSystem(numParticles, bUseVBO);
...
void initParams()
{
	...
	params->AddParam(new Param<float>("time step", timestep, 0.0, 1.0, 0.001, &timestep));
    ...
    params->AddParam(new Param<float>("sprite size", spriteSize, 0.0, 0.1, 0.001, &spriteSize));
    ...
    params->AddParam(new Param<float>("particle lifetime", particleLifetime, 0.0, 1000.0, 1.0, &particleLifetime));
}
...
main(int argc, char** argv) 
{
    ...
    if (g_bQAReadback) {
        ...
        initParticles(numParticles, false);
        initParams();
        ...
        initParticles(numParticles, true);
        initParams();
        initMenus();
