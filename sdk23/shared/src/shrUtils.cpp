    ...
    for (pStr = cFormatString; *pStr; ++pStr)
    {
        ...
        else 
        {
            ...
            switch (*++pStr)
            {
                ...
                case '.':  // float/double with specified precision and type field (f, e or E)
                {
                    // set dArg as the next value in list
                    dArg = va_arg(vaArgList, double);

                    // format for float/double with specified precision
                    char cFormat [5] = "%.3f";
                    cFormat[2] = *++pStr;   // jump past the decimal pt and get the precision width
                    cFormat[3] = *++pStr;   // jump past the precision val and get type field (f, e, E)   
