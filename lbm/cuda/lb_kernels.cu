	...
	const int nx, const int ny ) 
{
    ...
    //check false | true
    if ( (obst[x * nx] == 0) && ((lb[x * nx][3] - t_1) > 0) && 
                 ((lb[x * nx][6] - t_2) > 0) && 
		 (lb[x * nx][7] - t_2 > 0)) {
      //increase east
      lb[x * nx][1] += t_1;
...
__global__ void lb_bounceback_kernel( struct lb_d2q9 *lb,
		const struct lb_d2q9 *tmp, const int *obst,
		const int nx, const int ny )
{
	...
	if ( obst[ pos(x,y,nx) ] == 1 ){
		//east
		lb[ pos(x,y,nx) ][1] = tmp[ pos(x,y,nx) ][3];
		//north
		lb[ pos(x,y,nx) ][2] = tmp[ pos(x,y,nx) ][4];
		//west
		lb[ pos(x,y,nx) ][3] = tmp[ pos(x,y,nx) ][1];
		//south
		lb[ pos(x,y,nx) ][4] = tmp[ pos(x,y,nx) ][2];
		//north-east
		lb[ pos(x,y,nx) ][5] = tmp[ pos(x,y,nx) ][7];
		//north-west
		lb[ pos(x,y,nx) ][6] = tmp[ pos(x,y,nx) ][8];
		//south-west
		lb[ pos(x,y,nx) ][7] = tmp[ pos(x,y,nx) ][5];
		//south-east
		lb[ pos(x,y,nx) ][8] = tmp[ pos(x,y,nx) ][6];
	}
}

__global__ void lb_relaxation_kernel( 
		struct lb_d2q9 *lb, const struct lb_d2q9 *tmp,
		const int *obst, const int nx, const int ny,
		const float omega )
{
	...
	if ( obst[pos(x,y,nx)] == 0 ) {
		d_loc= 0.0;
		for (i = 0; i < l->n; i++)
			d_loc += tmp[ pos(x,y,nx) ];

		//x-, and y- velocity components
		u_x = (tmp[pos(x,y,nx)][1] + tmp[pos(x,y,nx)][5] + tmp[pos(x,y,nx)][8] -
				(tmp[pos(x,y,nx)][3] + tmp[pos(x,y,nx)][6] +
				 tmp[pos(x,y,nx)][7])) / d_loc;

		u_y = (tmp[pos(x,y,nx)][2] + tmp[pos(x,y,nx)][5] + tmp[pos(x,y,nx)][6] -
				(tmp[pos(x,y,nx)][4] + tmp[pos(x,y,nx)][7] +
				 tmp[pos(x,y,nx)][8])) / d_loc;
		...
		for (i = 0; i < l->n; i++)
			lb[pos(x,y,nx)][i] = tmp[pos(x,y,nx)][i]
			       	+ omega * (n_equ[i] - tmp[pos(x,y,nx)][i]);
	}
}
