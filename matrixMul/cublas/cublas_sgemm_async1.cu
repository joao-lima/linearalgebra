...
main(int argc, char** argv)
{
	...
	CUDA_SAFE_CALL( cudaStreamCreate( &stream ) );
    	CUBLAS_SAFE_CALL( cublasSetKernelStream( stream ) );
	...
	for( i= 0; i < max_iter; i++ ){
		...
		CUBLAS_SAFE_CALL( cublasGetVectorAsync( size_C,
					sizeof(h_C[0]), d_C,
					1, h_C, 1, stream) );
	...
	fprintf( stdout, "sgemm_pinned size= %d time(ms)= %.3f bandwidth(MB/s)= %.1f\n",
